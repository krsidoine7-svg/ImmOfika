...
Nous avons configuré plusieurs scénarios clés dans le dossier [scenarios](file:///c:/Users/Toto.ADMINISTRATOR/FavorCI/locust-load-tests/scenarios/) pour couvrir les cas d'utilisation du projet :

| Scénario | Fichier | Description / Utilité |
| :--- | :--- | :--- |
| **Navigation Publique** | [basic_test.py](file:///c:/Users/Toto.ADMINISTRATOR/FavorCI/locust-load-tests/scenarios/basic_test.py) | Visite de pages statiques anonymes (Accueil, À propos). Utile pour mesurer les performances du serveur web (Nginx/Apache) et de la mise en cache. |
| **Authentification Simple** | [auth_test.py](file:///c:/Users/Toto.ADMINISTRATOR/FavorCI/locust-load-tests/scenarios/auth_test.py) | Connexion d'un utilisateur au démarrage (`on_start`), récupération du jeton JWT et injection automatique dans les en-têtes d'autorisation (`Authorization: Bearer <token>`) pour les requêtes protégées suivantes. |
| **Sécurité Anti-CSRF** | [csrf_test.py](file:///c:/Users/Toto.ADMINISTRATOR/FavorCI/locust-load-tests/scenarios/csrf_test.py) | Extraction dynamique du jeton CSRF depuis le code HTML du formulaire via Regex (requis par Django, Laravel, Symfony, etc.) avant de soumettre une requête POST. |
| **Données Dynamiques (Multi-comptes)** | [dynamic_auth_test.py](file:///c:/Users/Toto.ADMINISTRATOR/FavorCI/locust-load-tests/scenarios/dynamic_auth_test.py) | Utilisation d'un fichier CSV ([users.csv](file:///c:/Users/Toto.ADMINISTRATOR/FavorCI/locust-load-tests/data/users.csv)) et d'une file d'attente (`Queue`) pour connecter des centaines d'utilisateurs réels différents sans collisions de sessions. |
| **Stress Base de Données (DB)** | [db_intensive_test.py](file:///c:/Users/Toto.ADMINISTRATOR/FavorCI/locust-load-tests/scenarios/db_intensive_test.py) | Simule des requêtes SQL lourdes (recherches à filtres aléatoires, calculs de statistiques agrégées et écritures en POST) pour tester les limites physiques du serveur de base de données. |
| **Recherche de Biens Favor** | [favor_biens_search_test.py](file:///c:/Users/Toto.ADMINISTRATOR/FavorCI/locust-load-tests/scenarios/favor_biens_search_test.py) | **(Spécifique Projet)** Simulation réaliste d'utilisateurs recherchant des biens immobiliers sur le site via des filtres de recherche complexes (bypass de cache `/api/biens`) et consultant les fiches de biens individuelles. |
