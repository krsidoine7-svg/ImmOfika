...
## [18 Mai 2026] Instanciation dynamique conditionnelle de Resend

**Contexte :** Gestion des clés API absentes dans les variables d'environnement locales (sans bloquer le démarrage de Next.js ni le chargement des Server Actions)  
**Décision :** Importation statique de `Resend` mais instanciation conditionnelle et dynamique (`new Resend(apiKey)`) à l'intérieur de l'action de traitement, uniquement si `RESEND_API_KEY` est présent.  
**Raison :** L'appel global au constructeur sans clé API provoquait un crash Next.js bloquant au démarrage (import time). Cette approche hybride garantit le démarrage de l'appli sans clé API configurée localement, tout en respectant les exigences strictes d'ESLint sur les imports statiques.  
**Alternative rejetée :** `require('resend')` dynamique (rejetée par ESLint) ou instanciation globale non sécurisée.  
**Lien :** [app/actions/reservation.ts](../../src/app/actions/reservation.ts)
