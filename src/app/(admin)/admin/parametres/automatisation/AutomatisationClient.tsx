...
export default function AutomatisationClient({ initialConfigs }: { initialConfigs: Record<string, any> }) {
  const [formData, setFormData] = React.useState({
    webhookUrl: initialConfigs.automations?.webhookUrl || '',
    resendApiKey: initialConfigs.automations?.resendApiKey || '',
    whatsappNotify: initialConfigs.automations?.whatsappNotify ?? false,
    emailNotify: initialConfigs.automations?.emailNotify ?? true,
    smsNotify: initialConfigs.automations?.smsNotify ?? false,
  })
  ...
  const handleSave = async () => {
    setIsSaving(true)
    try {
      const res = await saveSystemSettingsAction('automations', formData)
      if (res.success) {
        toast.success("Paramètres d'automatisation enregistrés avec succès !")
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
      <div className="p-6">
        <AutomationSettings values={formData} onChange={handleChange} />
      </div>
